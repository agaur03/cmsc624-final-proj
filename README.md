...
At any time, this will first compile the system; if this succeeds with no errors, it will also run two three scripts: two which performs some basic correctness tests of your implementation, and a third which profiles performance of the system. This third one takes a number of minutes to run, but you can cancel it at any time by pressing `ctrl-C`. 

> You can not pass the lock manager and transaction type tests right now until you complete the lock manager implementation.
...
* txn/txn_processor.cc: `RunLockingScheduler` method (Part 1C), `RunOCCScheduler` method (Part 2), `RunOCCParallelScheduler` method (Part 3), and`RunMVCCScheduler` method (Part 4).
* txn/mvcc_storage.cc: `Read` method, `Write` method, and `CheckWrite` method (Part 4).

However, to understand what's going on in the framework, you will need to look through most of the files in the `txn/` directory. We suggest looking first at the **TxnProcessor** class (`txn/txn_processor.h`) and in particular the **RunSerialScheduler()** methods and examining how it interacts with various objects in the system. It is critical to note that the transactions in this system require you to read both the *read* and *write* sets for execution.  

For each scheduler, the pseudo code does not assign the commit_id_, you need to analyze the code and determine the correct instance to update the commit_id_ for each transaction. The commit_id_ for aborted transactions is set to UINT64_MAX, and is trivial to implement. For committed transactions, you can use the `next_commit_id_` variable along with the `mutex_` lock to ensure that the commit_id_ is unique. The critical part is to figure out when is the appropriate time to assign the commit_id_ for a transaction depending on the scheduler used.
...
### Part 1C: Complete the Locking Scheduler (2 points)
Most of your locking scheduler is already implemented in the `RunLockingScheduler` method. However, you will need to correctly assign the commit_id_ for each transaction. The commit_id_ for an aborted transaction must be set to UINT64_MAX. The commit_id_. For committed transactions, you can use the `next_commit_id_` variable along with the `mutex_` lock to ensure that the commit_id_ is unique. The critical part is to figure out when is the appropriate time to assign the commit_id_ for a transaction.
...
- Does the MVCC pseudocode guarantee serializability? (we are asking specifically about the pseudocode from this assignment --- not about MVCC in general or the version from PostgreSQL we discussed in class). Please provide a brief explanation of why or why not (if not, please provide an example).
