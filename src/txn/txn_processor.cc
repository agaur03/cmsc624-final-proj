...
void TxnProcessor::RunCalvinScheduler() { 
    ...
    while (!stopped_.load()) {
        ...
        while (true) {
            ...
            auto elapsed = std::chrono::duration<float>(now - epoch_start).count();
        ...
        if (!current_epoch_txns.empty()) {
            ...
            for (Txn* txn : current_epoch_txns) {
                ...
                // Request write locks only if read locks succeeded
                if (!blocked) {
                ...
                if (!blocked) {
                    ...
                    for (const Key& key : acquired_read_locks) {
                        lm_->Release(txn, key);
                    }
                    
                    for (const Key& key : acquired_write_locks) {
                        lm_->Release(txn, key);
                    }
            ...
            // Wait for and process completed transactions
