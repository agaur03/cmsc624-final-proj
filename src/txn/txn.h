...
class Txn
{
   public:
    ...
    Txn() : status_(INCOMPLETE), commit_id_(0) {}
    virtual ~Txn() {}
    ...
    // Returns the transaction's commit ID. 
    uint64_t CommitId() { return commit_id_; }
...
#define ABORT                  \
    ...
    time_t occ_start_time;

    // Unique commit ID, assigned by TxnProcessor.
    uint64_t commit_id_;
};
